...
import comtest.utils.FileUtilities;
 ...
 * @author vesal
 */
public class ComTestScanner extends comtest.legacy.ComTestScanner { // NOPMD by vesal on 13.1.2008 15:29
    private final Strings methodNames = new Strings();    
     ...
     */
    public class CommentCodeHandler extends comtest.legacy.ComTestScanner.CommentCodeHandler { // NOPMD by vesal on 13.1.2008 15:29
        /**
        ...
        public void handleSentences(String line) { // NOPMD by vesal on 13.1.2008 15:29
            ...
            for ( int i=0; i<sentences.size(); i++ ) {
                ...
                p = indexOfNotInQuotes(s,ALMOSTMARKER1);  eq = ALMOSTMARKER1; tol = ", " + TOLERANCE;  // tolPostFix = "Delta"; /// ~~~
                if ( p < 0 ) { p = indexOfNotInQuotes(s,ALMOSTMARKER2); eq = ALMOSTMARKER2; }          /// ~~
                if ( p < 0 ) { p = indexOfNotInQuotes(s,EQUALSMARKER);  eq = EQUALSMARKER; tol = ""; tolPostFix = "";} /// ===
    ...
    protected void createTestFileBegin() throws IOException {
        ...
        Strings testContents = new Strings(); //getFileContents(outFileName);
...
//            addOutln("#include <iostream>");
        addOutln("class " + testClassName + " {");
        /*
        addOutln("#define assertEquals(s,e,g) { if ( assertEqualsImpl((s),(e),(g)) ) return; }");
        addOutln("");
        addOutln("template <class T1,class T2>");
        addOutln("  int assertEqualsImpl(const char *msg,T1 expected, T2 got) {");
        addOutln("    if ( expected == got ) return 0;");
        addOutln("    std::cout << msg << \": expected \" << expected << \" got \" << got << std::endl;");
        addOutln("    return 1;");
        addOutln("  }");
*/
    }
    ...
    protected void addHeader(PrintWriter out) {
        println(out,contents,"\n"," main(", " "+testClassName+"OrigMain("); // in C++ put the original file first
